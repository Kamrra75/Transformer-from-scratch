...
#COMPONENTS NEEDED FOR TRANSFORMER
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model:int, seq_len : int, dropout:float) -> None:
        ...
        #cretae a matrix of shape(seq_len, d_model)
        ...
        #Applt the sin to even positions
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        attention_scores = (query @ key.transpose(-2, -1)) / math.sqrt(d_k) #softmax(QK^T)/(sqrt(d_k))
        if mask is not None:
        ...
        return(attention_scores @ value), attention_scores #Multiplied with V
